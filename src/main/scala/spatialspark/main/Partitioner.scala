...
/**
 * Created by Simin You on 10/27/14.
 */
/**
 *
 */
object Partitioner {
  ...
  def main(args: Array[String]) {
    ...
    if (partitionMethod == PartitionMethod.FGP) {
      ...
      val STPartition = SortTilePartition(sc, inputMBRs.sample(false, ratio), extent, dimX, dimY, parallel)
      ...
      val BSPartition = BinarySplitPartition(sc, inputMBRs.sample(false, ratio), extent, level, parallel)
      partitions = BSPartition
    } else {
      //Not support..
    }

    //partitions.take(2).foreach(x => println(x.toString("\t")))
    val output = sc.parallelize(partitions, 1).zipWithIndex()
                   .map(x => x._2 + "\t" + x._1.toText()).saveAsTextFile(outputFile)
    /*
    sc.parallelize(STPartition, 1).zipWithIndex().map(x => x._2 + "\t"
      + "POLYGON((" + x._1.xmin + " " + x._1.ymin + ","
      + x._1.xmax + " " + x._1.ymin + ","
      + x._1.xmax + " " + x._1.ymax + ","
      + x._1.xmin + " " + x._1.ymax + ","
      + x._1.xmin + " " + x._1.ymin + "))").saveAsTextFile("/Users/you/data/stpart_test")
    */
